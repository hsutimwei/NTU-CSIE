...
void my_thrdstop_handler(void *arg){
    ...
    else
    {
        ...
        //printf("my_thrdstop_handler add %d\n",myarg);
...
void schedule(void){
    ...
    {   
        ...
        if(current_thread->is_exited == 1 || current_thread->is_yield == 1)
            current_thread = current_thread->next;
        else if (--current_thread->time_quantum == 0) {
            current_thread->time_quantum = 3;
            current_thread = current_thread->next;
        }
    }
    #endif
    ...
    {
        struct thread* t = current_thread, *mint = current_thread;

        while(t->next != current_thread)
        {
            t = t->next;
            if (t->remain_execution_time < mint->remain_execution_time || (t->remain_execution_time == mint->remain_execution_time && t->ID < mint->ID))
                mint = t;
        }
        current_thread = mint;
    ...
    {
        struct thread* t = current_thread->next, *mint = current_thread->next;
    ...
    else //if (is_thread_start == 0 || current_thread->is_yield == 1)
    {
        struct thread* t = current_thread, *mint = current_thread;
